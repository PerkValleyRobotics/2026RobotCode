...
public class Launcher extends SubsystemBase {
  ...
  public Launcher(LauncherIO io) {
    this.io = io;
    this.flywheel = LinearSystemId.createFlywheelSystem(turningSimMotor, FLYWHEEL_MOI_KGM, FLYWHEEL_GEARING);
    this.hood = LinearSystemId.createSingleJointedArmSystem(turningSimMotor, HOOD_MOI_KGM, HOOD_GEARING);

    // sysid configuration
    // this.sysId = new SysIdRoutine(, null))

    // filter and kqr
    this.flywheelObservFilter = new KalmanFilter<>(
        Nat.N1(), Nat.N1(), flywheel, VecBuilder.fill(3), VecBuilder.fill(0.03), 0.02);
    this.hoodObservFilter = new KalmanFilter<>(
