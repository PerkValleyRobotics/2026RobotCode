...
import static frc.robot.subsystems.launcher.LauncherConstants.SHOOTING_MOTOR_ID;
import static frc.robot.subsystems.launcher.LauncherConstants.SHOOTING_MOTOR_MAX_AMPERAGE;
import static frc.robot.subsystems.launcher.LauncherConstants.TURNING_MOTOR_ID;
import static frc.robot.subsystems.launcher.LauncherConstants.TURNING_MOTOR_MAX_AMPERAGE;
import static frc.robot.subsystems.launcher.LauncherConstants.TURN_MOTOR_kD;
import static frc.robot.subsystems.launcher.LauncherConstants.TURN_MOTOR_kI;
import static frc.robot.subsystems.launcher.LauncherConstants.TURN_MOTOR_kP;
...
public class LauncherIOSpark implements LauncherIO {
    ...
    public void updateInputs(LauncherIOInputs inputs) {
        ...
        ifOk(shootingMotor, shootingEncoder::getVelocity, (value) -> inputs.shootingMotorSpeedRadsPerSecond = value);
        ifOk(shootingMotor, new DoubleSupplier[] { shootingMotor::getBusVoltage, shootingMotor::getAppliedOutput },
                (value) -> inputs.shootingMotorAppliedVolts = value[0] * value[1]);
        ifOk(shootingMotor, shootingMotor::getOutputCurrent, (value) -> inputs.shootingMotorCurrentAmps = value);


        inputs.turningMotorConnected = turnDebounce.calculate(sparkStickyFault);

        ifOk(turningMotor, turningEncoder::getPosition, (value) -> inputs.turningMotorPositionDeg = value);
        ifOk(turningMotor, new DoubleSupplier[] { turningMotor::getBusVoltage, turningMotor::getAppliedOutput },
                (value) -> inputs.turningMotorAppliedVolts = value[0] * value[1]);
        ifOk(turningMotor, turningMotor::getOutputCurrent, (value) -> inputs.turningMotorCurrentAmps = value);
