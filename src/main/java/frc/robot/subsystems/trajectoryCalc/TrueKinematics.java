...
public class TrueKinematics implements LauncherTrajectoryCalc {
    ...
    double D = Math.pow(inputs.launchVelocity(), 4) - (9.8)*((9.8 * Math.pow(distance, 2)) + (2*HUB_TRIANGULATED_Z*Math.pow(inputs.launchVelocity(), 2)));
    if (D < 0){return 0;}
    return Math.atan(Math.pow(inputs.launchVelocity(), 2) + Math.sqrt(D)/(distance * 9.8));
    }
}
